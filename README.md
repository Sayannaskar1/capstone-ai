...
### 🔍 FAISS + Sentence Transformers (RAG — available but bypassed for speed)
**RAG** stands for *Retrieval-Augmented Generation*. The idea is simple: instead of feeding the entire document to the LLM (which would be too slow and too long), you:
...
**FAISS** (Facebook AI Similarity Search) is an ultra-fast library for finding similar vectors among millions of them in milliseconds.
